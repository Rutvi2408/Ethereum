var RPToken = artifacts.require("./RPToken");

contract ('RPToken', function (accounts) {

	var tokenInstance; 

	it('initializes the contract with correct values', function(){
		return RPToken.deployed().then(function(instance){
			tokenInstance = instance;
			return tokenInstance.name();
		}).then(function(name){
			assert.equal(name,'RP Token','has the correct name');
			return tokenInstance.symbol();
		}).then(function(symbol){
			assert.equal(symbol,'RP','has the correct symbol');
			return tokenInstance.standard();
		}).then(function(standard){
			assert.equal(standard,'RP Token v1.0','has the correct standard');
		});
	})
...
it('transfer token ownership', function(){
	return RPToken.deployed().then(function(instance){
		...
		assert.equal(balance.toNumber(),250000, 'add the amount to the recieving account');
		return tokenInstance.balanceOf(accounts[0]);
	}).then(function(balance){
